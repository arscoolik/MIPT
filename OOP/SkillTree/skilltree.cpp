...
public:
    ...
    int onMousePressed(sf::Vector2f mouseCoords, sf::Mouse::Button mouseButton)
    {
        if (mState == State::Blocked)
            return 0;
        int pointsChange = 0;
...
public:
    WarriorSkillTree(float rootXPosition) : SkillTree{rootXPosition}
    {
        ...
        root->addChild(eye);
        root->addChild(shuriken);
        eye->addChild(std::shared_ptr<Node>{new FreezeAccumulativeSkillNode({rootXPosition - 100, 200})});
        shuriken->addChild(std::shared_ptr<Node>{new SpikesSkillNode({rootXPosition + 100, 200})});
...
public:
    RogueSkillTree(float rootXPosition) : SkillTree{rootXPosition}
    {
        ...
        root->addChild(eye);
        root->addChild(shuriken);
        eye->addChild(std::shared_ptr<Node>{new SwordAccumulativeSkillNode({rootXPosition - 100, 200})});
        shuriken->addChild(std::shared_ptr<Node>{new SpikesSkillNode({rootXPosition + 100, 200})});
...
int main()
{
    ...
    sf::RenderWindow window(sf::VideoMode(1500, 800), "Skill Tree", sf::Style::Close, settings);
